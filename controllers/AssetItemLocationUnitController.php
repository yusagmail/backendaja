 ...
 */
class AssetItemLocationUnitController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionIndex($i="", $t="")
    {
        $searchModel = new AssetItemSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        $folder = $this->getFolder();
        return $this->render($folder.'/index', [
    ...
    public function actionViewStock($i, $t="", $action="index", $idi="")
    {
        $id = \common\utils\EncryptionDB::encryptor('decrypt', $idi);
        $idm = \common\utils\EncryptionDB::encryptor('decrypt', $i);
        // var_dump($this->findModelMaster($id));die;
        $folder = $this->getFolder();

        return $this->render($folder.'/view-stock', [
            'model' => $this->findModel($id),
            'model_master' => $this->findModelMaster($idm),
            'i' => $i,
            'action' => $action,
            'idi' => $idi,
            't' => $t,
    ...
    public function actionGenerateBarcode($i, $label)
    {
        $id = \common\utils\EncryptionDB::encryptor('decrypt',$i);
        $model = $this->findModel($id);
        return $this->render('barcode/generate-barcode-material', [
            'id' => $id,
            'model' => $model,
        ]);
    }

    public function actionGenerateQrcode($i, $label)
    {
        $id = \common\utils\EncryptionDB::encryptor('decrypt',$i);
        $model = $this->findModel($id);
        return $this->render('qrcode/generate-qrcode', [
            'id' => $id,
            'model' => $model,
        ]);
    }

    public function actionGenerateQris($i, $label)
    {
        $id = \common\utils\EncryptionDB::encryptor('decrypt',$i);
        $model = $this->findModel($id);
        return $this->render('qrcode/generate-qris', [
            'id' => $id,
            'model' => $model,
        ]);
    }
    ...
    public function actionCreate()
    {
        ...
        if ($modelLocation->load($postData)) {
            ...
            if ($model->load($postData)) {
                ...
                Yii::$app->response->redirect(['asset-item-search/index']);
                //return $this->redirect(['view', 'id' => $model->id_asset_item]);
            }
        }

        $folder = $this->getFolder();
        return $this->render($folder.'/create_single', [
            'model' => $model,
    ...
    public function actionUpdateAsset($c)
    {
        ...
        return $this->render('update-asset', [
            'model' => $model,
            'modelLocation' => $modelLocation,
            'modelReceived' => $modelReceived,
        ]);
 ...
 */
    ...
    protected function findModelMaster($id)
    {
