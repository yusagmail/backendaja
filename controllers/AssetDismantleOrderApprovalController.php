 ...
 */
class AssetDismantleOrderApprovalController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionReceived($i){
        ...
        $modelReceived->approval_date = \common\helpers\Timeanddate::getCurrentDate();

        $modelOrder = $this->findModel($id);

        if ($modelReceived->load(Yii::$app->request->post()) && $modelReceived->save()) {
            //Generate Automatic User ID
            $modelReceived->approval_user_id = Yii::$app->user->identity->id;
