 ...
 */
class FindByUnitController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionGetListLocationUnit3($idp)
    {
        // Join AssetItemLocationUnit with AssetItem to fetch related asset items
        $assetItems = \backend\models\AssetItemLocationUnit::find()
        ->joinWith('assetItem')
        ->where(['asset_item_location_unit.id_location_unit' => $idp])
        ->all();

            
        $assets = [];
        foreach ($assetItems as $item) {
            $idi = EncryptionDB::encryptor('encrypt', $item->assetItem->id_asset_item);
            $i = EncryptionDB::encryptor('encrypt',$item->assetItem->id_asset_master);
            $assets[] = [
                'id' => $item->id_asset_item,
                'i' => $i,
                'idi' => $idi,
                'keterangan' => $item->assetItem->assetMaster->asset_name,
                'type_asset' => $item->assetItem->assetMaster->typeAsset1->type_asset ?? '(not set)',
                'number1' => $item->assetItem->number1,
            ];
