...
<?php
    ...
    function singlePriceToAllDisplay(){
        $dis = '
        <button class="btn btn-sucess btn-sm" data-toggle="collapse" data-target="#demo2">
          Advance <i class="fa fa-level-down"></i>
        </button>

        <div id="demo2" class="pengajauan-keberatan-search collapse" style="padding: 10px; background-color: #f7f7f7; border: 1px solid #d7d7d7">
            Single Price - Satu Harga untuk semua
            <input type="text" name="singlePrice" class="form-control" maxlength="12" value="0"  id="txtSinglePrice" onchange="copyToAll()">
        </div>
        ';
        ...
        <?= GridView::widget([
            ...
            */
            'columns' => [
                ...
                */
                [
                    //'label' => 'Harga Jual<br><input type="button" value="Advanced">'.singlePriceToAllDisplay(),
                    'label' => 'Harga Jual &nbsp;'.singlePriceToAllDisplay(),
                    'format' => 'raw',
                    'encodeLabel' => false,
                    'value' => function ($data, $key, $index, $column) {
                        $i = \common\utils\EncryptionDB::encryptor('encrypt',$data->id_material_sales);
                        $val = $data->sales_harga_jual;
                        return '<input type="text" name="asas" class="form-control" maxlength="12" value="'.$val.'" onchange="sendHargaJualIdx(\''.$i.'\','.$data->yard.','.$index.')" id="txtHarga_'.$index.'">
                        ';
                    },
                    //'filter'=>Html::activeDropDownList($searchModel, 'id_material_kategori2', $dataListMaterialKategori2, ['class' => 'form-control']),
                ],
...
<script>
function sendHargaJualIdx(i,y, idx){
  //alert(i);
  $( "#totalHarga_" + i ).html( "..." );
  var msgText = $( "#txtHarga_" + idx ).val();
  //alert(msgText);
  if(msgText != ""){
      var newval = parseInt(msgText);
        if(newval >0){
...
function copyToAll(){
  ...
  var msgText = $( "#txtSinglePrice" ).val();
  //alert(msgText);
  if(msgText != ""){
      var newval = parseInt(msgText);
        if(newval >0){
            for(let i=0;i<=<?= $totalData ?>;i++){
                $("#txtHarga_"+i).val( newval );
                $('#txtHarga_'+i).trigger('change');
            }
        }else{
            alert('Silakan isi dengan angka!');
        }
