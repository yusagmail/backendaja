...
<script>
  ...
  function moveCube(dx, dy) {
    ...
    controls.target.set(centerX, centerY, 0); // Pusat rotasi tetap di (10, 15)
    controls.update();
  ...
  document.getElementById("moveUp").addEventListener("click", function () {
    moveCube(0, 1); // Geser kubus ke atas
  });

  document.getElementById("moveDown").addEventListener("click", function () {
    moveCube(0, -1); // Geser kubus ke bawah
  });
  ...
  var touchMove = (event) => {
    if (event.touches.length === 2 && initialAngle !== null) {
      ...
      var pivot = new THREE.Vector3(centerX, centerY, 0);
