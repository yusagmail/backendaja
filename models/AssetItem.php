...
class AssetItem extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['id_asset_master', 'id_asset_received', 'id_asset_item_location',
                ...
                'id_type_asset_item4', 'id_type_asset_item5', 'id_customer'], 'integer'],
    ...
    {
        return $this->hasOne(AssetItemLocationUnit::className(), ['id_asset_item' => 'id_asset_item']);
    }
...
//                Model Penamaan dengan default name dan id saja (agar kalau ada file baru langsung timpa di ID yang sama)
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            // Generate Kode Barang and Nomor Inventaris if they are empty
            if ($this->isNewRecord || empty($this->number1)) {
                $this->generateKodeBarang();
            }
            if ($this->isNewRecord || empty($this->number2)) {
                $this->generateNomorInventaris();
            }
            return true;
        }
        return false;
    ...
    /**
     * Generate Kode Barang based on Nama Barang
     */
    protected function generateKodeBarang()
    {
        $this->number1 = 'KB-' . strtoupper(substr($this->number3, 0, 3)) . '-' . sprintf('%04d', $this->getMaxId() + 1);
    }

    /**
     * Generate Nomor Inventaris based on Nama Barang
     */
    protected function generateNomorInventaris()
    {
        $this->number2 = 'INV-' . strtoupper(substr($this->number3, 0, 3)) . '-' . sprintf('%04d', $this->getMaxId() + 1);
    }

    /**
     * Get the maximum id_asset_item from the table
     * @return int
     */
    protected function getMaxId()
    {
        return (int) self::find()->max('id_asset_item');
    }
